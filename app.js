...
// Ustawienie nicku od razu po załadowaniu
const usernameEl = document.getElementById('username');
if (usernameEl) {
    usernameEl.textContent = myUser + " ✏️";
}

if (["admin", "kris", "krys", "szym", "sim"].some(n => myUser.toLowerCase().includes(n))) {
...
function renderAvatar(avatarValue) {
    const imgEl = document.getElementById('my-avatar-img');
    const txtEl = document.getElementById('my-avatar');
    ...
    if (isUrl && imgEl && txtEl) {
        imgEl.src = avatarValue;
        imgEl.style.display = 'block';
        txtEl.style.display = 'none';
    } else if (imgEl && txtEl) {
        imgEl.style.display = 'none';
        txtEl.style.display = 'inline';
        txtEl.innerText = avatarValue;
    }
}
...
window.openProfileMenu = async function() {
    ...
    const choice = prompt(
        "CO CHCESZ EDYTOWAĆ?\n" +
        "1. Wgraj zdjęcie z urządzenia (kompresja do 256x256)\n" +
        "2. Wpisz link URL / Emoji\n" +
        "3. Zmień opis (Bio)"
    );
    ...
    if (choice === "1") {
        ...
        const newAv = prompt("Wklej link do zdjęcia (URL) lub wpisz emoji:");
        if (newAv !== null && newAv.trim() !== "") {
            const cleanAv = newAv.trim();
            renderAvatar(cleanAv);
            await setDoc(doc(db, "users", myUser), { avatar: cleanAv }, { merge: true });
        }
    } else if (choice === "3") {
        const bioEl = document.getElementById('my-bio');
        const currentBio = bioEl ? bioEl.innerText.replace(/"/g, '') : "";
...
window.handlePfpUpload = function(event) {
    ...
    reader.onload = function(e) {
        ...
        img.onload = function() {
            ...
            renderAvatar(base64Image);
            setDoc(doc(db, "users", myUser), { avatar: base64Image }, { merge: true })
                .then(() => alert("Profilowe zostało pomyślnie zaktualizowane!"))
...
async function loadStats() {
    try {
        ...
        const regEl = document.getElementById('reg-count');
        if (regEl) regEl.innerText = snapshot.data().count;

        const fiveMinsAgo = new Date(Date.now() - 5 * 60 * 1000);
        const q = query(collection(db, "users"), where("lastActive", ">", fiveMinsAgo));
        
        onSnapshot(q, (snap) => {
            const onEl = document.getElementById('on-count');
            if (onEl) onEl.innerText = snap.size;
        });
    } catch(e) {}
}
loadStats();
...
if(myUser) {
    ...
    onSnapshot(qInv, (snap) => {
        ...
        snap.forEach(docBox => {
            ...
            const accBtn = document.getElementById(`acc-${reqId}`);
            if (accBtn) {
            ...
            const rejBtn = document.getElementById(`rej-${reqId}`);
            if (rejBtn) {
...
window.openDM = async function(targetUser) {
    ...
    const modalEl = document.getElementById('dm-modal');
    if (titleEl) titleEl.innerText = "Czat z: " + targetUser;
...
function applyTheme(theme) {
    ...
    if(fInput) { 
        fInput.style.borderColor = (theme === 'premium' ? "#ddd" : theme === 'dark' ? "#444" : "#00ffff"); 
        fInput.style.background = (theme === 'premium' ? "#fff" : theme === 'dark' ? "#252525" : "#000"); 
        fInput.style.color = (theme === 'premium' ? "#000" : "#fff"); 
    }
    if(uInput) { 
        uInput.style.borderColor = (theme === 'premium' ? "#ddd" : theme === 'dark' ? "#444" : "#00ffff"); 
        uInput.style.background = (theme === 'premium' ? "#fff" : theme === 'dark' ? "#252525" : "#000"); 
        uInput.style.color = (theme === 'premium' ? "#000" : "#fff"); 
    }
}

// Zabezpieczenie na przypadek braku okna ciasteczek w HTML
const cookieOverlay = document.getElementById('cookie-overlay');
if (cookieOverlay && localStorage.getItem('cookiesAccepted') === 'true') {
...
function rejectCookies() {
    const init = document.getElementById('cookie-initial');
    const pay = document.getElementById('pay-screen');
...
function showRandomPage() {
    ...
    if(random && random.href && random.href.trim() !== "") {
...
function toggleMusic() {
    ...
    if(musicPlaying){
        normalMusic.pause();
        if (mBtn) mBtn.textContent = "🎵 Muzyka";
    } else {
        normalMusic.play();
        if (mBtn) mBtn.textContent = "⏸️ Pauza";
    }
